...
import { planArticleVisuals, ProductVisualContext } from "./visual-planner";
import { composeBlogImages } from "./blog-image-composer";
...
{
  "mailchimp": {
    "subjectA": "Asunto técnico directo con gancho para instaladores",
    "subjectB": "Asunto alternativo orientado a costes y rentabilidad",
  ...
  "linkedin": {
    "hook": "¿Cómo dimensionar esta arquitectura de red sin cuellos de botella?",
    "body": "Desarrollo técnico de 3 párrafos explicando el desafío de ingeniería...",
    "takeaways": [
      "Punto clave 1: Rendimiento y optimización de ancho de banda",
      "Punto clave 2: Presupuesto PoE y eficiencia energética",
      "Punto clave 3: TCO sin cuotas de licencias obligatorias"
    ],
    "callToAction": "¿Cómo estás dimensionando tus despliegues? Abrimos debate en comentarios.",
    "hashtags": ["#Networking", "#Telecomunicaciones", "#EcomShop", "#B2B"],
...
`;
  ...
  try {
    ...
    // Contexto real del producto (Product Truth)
    const effectiveProductContext: ProductVisualContext = productContext || {
      sku: category ? category.toUpperCase() : "PROD-1",
      name: article.title,
      brand: "EcomShop",
    ...
    const visualPlan = planArticleVisuals(
      article.sections.map((s, idx) => ({
        ...
        title: s.title,
        contentType: s.contentType || s.level,
        bodyHtml: s.htmlContent
      })),
      effectiveProductContext
    );

    // Circuito completo P0: Junia Visual -> Image Engine -> Blog Composer -> Final HTML
    const compositionResult = await composeBlogImages(
      visualPlan.htmlWithSlots,
      visualPlan.photoPlacements,
      effectiveProductContext,
      apiKeyOverride
    );
    ...
    return {
      ...
      slug: article.slug,
      readingTimeMinutes,
      targetKeywords: [category, "Networking", "Telecomunicaciones B2B"],
      htmlContent: compositionResult.finalHtml,
      cleanPlainTextExcerpt: plainTextExcerpt,
      editorialLayout: {
        targetProfiles: [
          {
            profile: "Instalador",
            keyTakeaway: "Instalación simplificada con aprovisionamiento directo y stock garantizado."
          },
          {
        ...
        photoPlacements: compositionResult.updatedPlacements,
        ctaPlacements: [
          {
            id: "cta-1",
  ...
  // Contexto real del producto (Product Truth)
  const fallbackProductContext: ProductVisualContext = productContext || {
    sku: category ? category.toUpperCase() : "PROD-1",
    name: article.title,
    brand: "EcomShop",
    deviceType: "Dispositivo de Red Profesional"
  ...
  const fallbackVisualPlan = planArticleVisuals(
    article.sections.map((s, idx) => ({
      id: s.sectionId || `sec-${idx + 1}`,
      title: s.title,
      contentType: s.contentType || s.level,
  ...
  const fallbackComposition = await composeBlogImages(
    fallbackVisualPlan.htmlWithSlots,
    fallbackVisualPlan.photoPlacements,
    fallbackProductContext,
    apiKeyOverride
  );
  ...
  return {
    ...
    blog: {
      ...
      slug: article.slug,
      readingTimeMinutes,
      targetKeywords: [category, "Networking", "EcomShop B2B"],
      htmlContent: fallbackComposition.finalHtml,
      cleanPlainTextExcerpt: plainTextExcerpt,
      editorialLayout: {
        targetProfiles: [
          {
          ...
          {
            profile: "Director TIC",
            keyTakeaway: "Máxima estabilidad con enlaces Multi-Gigabit y gestión centralizada."
          }
        ],
        photoPlacements: fallbackComposition.updatedPlacements
      }
    },
